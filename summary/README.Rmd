...
plot_volcano <- function(dat, mtorc1_genes, mtorc2_genes, mtorc3_genes, target, plot_title,
  ...
  # split dat into two groups to allow shared significant genes to be made more visible?
  # dat1 <- dat %>%
  #   filter(group %in% c(complex_name, "Not significant"))
  #
  # dat2 <- dat %>%
  #   filter(!group %in% c(complex_name, "Not significant"))
  ...
  if (opts_knit$get("rmarkdown.pandoc.to") == "html") {
    ...
    plot_ly(data = dat,
      ...
      #add_trace(data = dat2, marker = list(size = 5), type = 'scatter',
