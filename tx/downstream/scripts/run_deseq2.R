...
contrasts2 = c('condition_m7_starved_vs_m7_nutri',
...
for (contrast in contrasts2) {
  ...
  parts <- unlist(strsplit(sub('condition_', '', contrast), "_vs_"))

  cond1 <- parts[1]
  cond2 <- parts[2]

  # res2 <- lfcShrink(dds2, coef = contrast, type = "ashr") %>%
  res2 <- lfcShrink(dds2, contrast=c("condition", cond1, cond2), type = "ashr") %>%
    as.data.frame() %>%
    rownames_to_column("ensgene")
